...
export class SqlJupyterKernel {
    ...
    if (result.columns.length === 0) {
      return new vscode.NotebookCellOutput([
        vscode.NotebookCellOutputItem.text(
          `[${connLabel} / ${dbLabel}] Query executed successfully. ${result.rowCount} row(s) affected. (${result.executionTime}ms)`
        )
      ]);
    }
  ...
  private renderHtmlTable(result: SqlResultSet, connLabel: string, dbLabel: string): string {
    ...
    const contextLine = connLabel
      ? `<div class="sql-context">${this.escapeHtml(connLabel)} | ${this.escapeHtml(dbLabel)}</div>`
      : '';
